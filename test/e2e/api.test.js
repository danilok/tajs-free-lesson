...
describe('API E2E test Suite', () => {
  ...
  describe('/login', () => {
    it('should receive not authorized when user or password is invalid', async() => {
      ...
      assert.strictEqual(
        result.status,
        expected, `status code should be 401: ${result.status}`
      )
    ...
    it('should login successfully given user and password', async() => {
      ...
      assert.strictEqual(
        result.status,
        expected, `status code should be 200: ${result.status}`
      )

      const response = await result.json()
      assert.ok(
        response.token.length > 20,
        `response.body should be a valid jwt token, actual: ${JSON.stringify(response.token)}`
      )
  ...
  describe('/', () => {
    it('should not be allowed to access private data without a token', async() => {
      ...
      assert.strictEqual(
        result.status,
        expected, `status code should be 400: ${result.status}`
      )
